        ...
        <div class="mt-16 flex justify-center">
            <div class="w-full lg:w-3/4 sm:w-full">
                ...
                <div class="overflow-x-auto mt-6 bg-white shadow-lg rounded-lg border border-gray-200 p-4">
                    <div class="mb-4">
                        <a href="{{ route('pemerintah.master_data.tambah-petugas') }}" class="bg-green-600 text-white px-4 py-2 rounded">Tambah Petugas</a>
                    </div>
                    <table class="w-full table-auto border-collapse">
                        <thead class="bg-gray-300">
                            <tr>
                                <th class="py-4 px-6 text-left text-gray-700 text-lg border-b border-gray-400">Nama Petugas</th>
                        ...
                        <tbody id="table-body" style="border-collapse: collapse; empty-cells: hide;">
                            @foreach($petugas as $petugasItem)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td class="py-4 px-6 text-gray-600 max-w-xs line-clamp-2">{{ $petugasItem->nama_petugas }}</td>
                                    <td class="py-4 px-6 text-gray-600 max-w-xs line-clamp-2">{{ $petugasItem->kecamatan->nama_kecamatan }}</td>
                                    <td class="py-4 px-6 text-gray-600 space-x-2">
                                        <ul>
                                            <li>
                                                <a href="{{ route('pemerintah.master_data.edit-petugas', $petugasItem->id) }}" class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-4 rounded">Edit</a>
                                            </li>
                                            <br>
                                            <li>
                                                <form action="{{ route('pemerintah.master_data.delete-petugas', $petugasItem->id) }}" method="POST" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
